...
void MSJExceptionHandler::IntelStackWalk( PCONTEXT pContext )
{
    ...
    do
    {
        ...
        if (SymFromAddr(hProcess, pc, 0, pSymbol)) {
          wchar_t szSymbolName[1024] = {0};
          MultiByteToWideChar(CP_ACP, 0, &pSymbol->Name[0], strlen(&pSymbol->Name[0]), 
            szSymbolName, 1024);
          _tprintf(_T("%08X  %08X  %04X:%08X %s!%s\n"), pc, pFrame, section, 
            offset, PathFindFileName(szModule), szSymbolName);
        } else {
          _tprintf(_T("%08X  %08X  %04X:%08X %s\n"), pc, pFrame, section, 
            offset, PathFindFileName(szModule));
        }
